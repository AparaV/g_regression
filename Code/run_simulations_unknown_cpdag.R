##
## Simulations with CPDAG estimation script
##
...
while (iters < num_reps) {
    ...
    out <- tryCatch({
        ...
        # Estimate CPDAG from data
        X_score <- new("GaussL0penObsScore", X)
        ges_fit <- pcalg::ges(X_score)
        cpdag_est_bool <- as.numeric(as(ges_fit$essgraph, "matrix"))
        cpdag_est <- matrix(cpdag_est_bool, nrow=num_nodes)
        colnames(cpdag_est) <- rownames(cpdag_est) <- colnames(X)
        
        # Sample treatments and outcomes
        cat("\tSampling treatment and outcome variables\n")
        AY <- sample_treatment_outcome(dag_object$dag_amat, dag_object$cpdag_amat, A_len, cpdag_est)
        if (is.null(AY)) {
